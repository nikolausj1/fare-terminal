...
export default function MarketError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col items-start gap-3 px-4 py-16 sm:px-6">
      <h1 className="text-xl font-semibold text-[var(--text-primary)]">Couldn&apos;t load this market</h1>
      <p className="text-sm text-[var(--text-secondary)]">Something went wrong fetching this route&apos;s data. This is usually temporary.</p>
      <button
        type="button"
        onClick={reset}
